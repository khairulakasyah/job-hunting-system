    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setEventModalOpen(true)}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-orbit-primary hover:bg-orbit-primary/90 text-white text-xs font-medium rounded-lg transition-colors">
      ...
      {timelineEvents.length === 0 ? (
        ...
        <div className="space-y-2">
          {upcomingEvents.map(ev => (
            <div key={ev.id} className="flex items-start gap-3 p-3 rounded-lg bg-orbit-primary/5 border border-orbit-primary/20">
              <div className="w-8 h-8 rounded-lg bg-orbit-primary/10 text-orbit-primary-light flex items-center justify-center flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                {ev.meeting_link && (
                  <a href={ev.meeting_link} target="_blank" rel="noopener noreferrer"
                    className="text-[11px] text-orbit-primary-light hover:text-orbit-accent transition-colors inline-block mt-0.5 truncate max-w-full">{ev.meeting_link}</a>
                )}
              </div>
            </div>
          ))}
          {pastEvents.length > 0 && (
            <>
              ...
              {pastEvents.map(ev => (
                <div key={ev.id} className="flex items-start gap-3 p-3 rounded-lg bg-orbit-surface2 border border-orbit-border opacity-60">
                  <div className="w-8 h-8 rounded-lg bg-orbit-surface3 flex items-center justify-center flex-shrink-0 text-slate-500"><Calendar className="w-4 h-4" /></div>
                  <div className="flex-1 min-w-0">
      ...
      <div className="pt-4 border-t border-orbit-border">
        ...
        {prepLoading ? (
          <div className="h-24 bg-orbit-surface2 rounded-lg animate-pulse" />
        ) : (
          <>
