    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div role="dialog" aria-modal="true" className="w-full max-w-4xl bg-orbit-surface border border-orbit-border rounded-2xl shadow-2xl pointer-events-auto flex flex-col max-h-[90vh]"
              ...
              <div className="px-6 border-b border-orbit-border flex-shrink-0">
                  ...
                  {TABS.map(tab => (
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)} role="tab" aria-selected={activeTab === tab.key}
                      className={`pb-3 pt-3 text-sm font-medium border-b-2 transition-colors ${
                        activeTab === tab.key
                          ? 'text-orbit-primary-light border-orbit-primary-light'
